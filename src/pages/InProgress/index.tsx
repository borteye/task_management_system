...
const InProgress = () => {
  ...
  const inProgressTasks = tasks.filter(task => task.stage.toLowerCase() === "in progress");
  const filteredTasks = filterTasks(inProgressTasks, search);

  return (
    <div className="pt-28 md:pt-36 p-4 flex flex-col gap-y-10 relative h-full overflow-y-auto">
      <h1 className="text-2xl sm:text-3xl font-poppins-medium">In Progress Tasks</h1>
      ...
      <div className={`${inProgressTasks.length === 0 ? "flex items-center justify-center" : "grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4"}`}>
